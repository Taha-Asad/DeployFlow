...
const GenerateDocsCommand_1 = require("./commands/GenerateDocsCommand");
const RollbackCommand_1 = require("./commands/RollbackCommand");
...
async function activate(context) {
    ...
    const secretManager = new SecretManager_1.SecretManager(context.secrets);
    ...
    const workflowEngine = new WorkflowEngine_1.WorkflowEngine(configManager, secretManager, logger);
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(ProgressPanel_1.ProgressPanel.viewType, progressPanel));
    ...
    const deployDisposable = vscode.commands.registerCommand("deployflow.deploy", async () => {
    ...
    const analyzeDisposable = vscode.commands.registerCommand("deployflow.analyze", async () => {
        const command = new AnalyzeCommand_1.AnalyzeCommand(configManager);
        await command.execute();
    });
    // GENERATE DOCS COMMAND — just generate SDLC documentation
    const generateDocsDisposable = vscode.commands.registerCommand("deployflow.generateDocs", async () => {
        const command = new GenerateDocsCommand_1.GenerateDocsCommand(configManager, secretManager);
        await command.execute();
    });
    // ROLLBACK COMMAND — restore previous deployment
    const rollbackDisposable = vscode.commands.registerCommand("deployflow.rollback", async () => {
        const command = new RollbackCommand_1.RollbackCommand(workflowEngine, secretManager);
        await command.execute();
    });
    // CONFIGURE COMMAND — interactive setup wizard
    const configureDisposable = vscode.commands.registerCommand("deployflow.configure", async () => {
        const command = new ConfigureCommand_1.ConfigureCommand(configManager, secretManager);
        await command.execute();
    });
    // SHOW PROGRESS COMMAND — open the progress panel
    const showProgressDisposable = vscode.commands.registerCommand("deployflow.showProgress", () => {
        progressPanel.show();
    });
    ...
    context.subscriptions.push(deployDisposable, analyzeDisposable, generateDocsDisposable, rollbackDisposable, configureDisposable, showProgressDisposable, logger);
    // ── Step 9: Welcome & AI Configuration Prompt ──────────────────────────
    // Show welcome message on first activation
    const hasSeenWelcome = context.globalState.get("hasSeenWelcome", false);
    if (!hasSeenWelcome) {
        const action = await vscode.window.showInformationMessage("🚀 DeployFlow AI is ready! Let's get you set up.", "Configure AI & Deployment", "Maybe Later");
    ...
    if (aiProvider !== "ollama") {
        ...
        if (!key) {
            const fix = await vscode.window.showWarningMessage(`⚠️ AI provider "${aiProvider}" is selected but no API key is set. ` +
                "The AI error fixer will fail until you configure it.", "Configure AI Now", "Switch to Ollama (no key needed)");
        ...
        if (!ollamaInstalled) {
            const install = await vscode.window.showWarningMessage("⚠️ Ollama is not installed. The AI error fixer needs it.", "Install Ollama", "Open Download Page");
...
// ─────────────────────────────────────────────────────────────────────────────
function deactivate() {
    Logger_1.Logger.getInstance().info("👋 DeployFlow AI deactivated");
}
