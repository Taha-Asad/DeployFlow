...
// This is the DATA STRUCTURE that describes a project
// Every field here is something the analyzer tries to detect
export interface ProjectInfo {
  ...
  // What framework? (React, Vue, Django, etc.)
  framework: string;
  ...
  // What command builds the project? e.g., 'npm run build'
  buildCommand: string;

  // What command starts the app? e.g., 'npm start'
  startCommand: string;

  // What port does the app listen on? e.g., 3000
  port: number;

  // The name of the app (from package.json or folder name)
  name: string;

  // What version? (from package.json)
  version: string;

  // Runtime version needed (e.g., '18' for Node 18, '3.11' for Python 3.11)
  runtimeVersion: string;

  // Is this a monorepo (many apps in one folder)?
  isMonorepo: boolean;

  // Which monorepo tool?
  monorepoTool?: "nx" | "turborepo" | "lerna";

  // List of packages in the monorepo (if applicable)
  workspaces?: string[];

  // Does the project have a Dockerfile already?
  hasDockerfile: boolean;

  // Does the project have docker-compose.yml?
  hasDockerCompose: boolean;

  // The main entry file (e.g., 'src/index.ts', 'app.py')
  entryPoint: string;

  // Key files found in the project
  files: {
    packageJson?: unknown;
  ...
  // Environment variables the app needs (found in .env.example)
  envVars: string[];
  ...
  // Any special notes or warnings for the user
  warnings: string[];
}
...
export class ProjectAnalyzer {
  ...
  public async analyze(projectPath: string): Promise<ProjectInfo> {
    ...
    this.logger.info(
      `✅ Project analysis complete: ${info.framework} (${info.language})`,
    );
  ...
  private async detectLanguageAndRuntime(
    projectPath: string,
    info: ProjectInfo,
  ): Promise<void> {
    ...
    const packageJson =
      await this.fileUtils.readJson<Record<string, unknown>>(packageJsonPath);

    if (packageJson) {
      info.files.packageJson = packageJson;
      info.name = (packageJson.name as string) || info.name;
      info.version = (packageJson.version as string) || info.version;
      ...
      const nvmrc = await this.fileUtils.readFile(
        path.join(projectPath, ".nvmrc"),
      );
      const nodeVersion =
        nvmrc ||
        (packageJson.engines as Record<string, string>)?.node?.replace(
          /[^0-9.]/g,
          "",
        ) ||
        "18";
    ...
    const requirementsTxt = await this.fileUtils.readFile(
      path.join(projectPath, "requirements.txt"),
    );
    const pyprojectToml = await this.fileUtils.readFile(
      path.join(projectPath, "pyproject.toml"),
    );
    const setupPy = await this.fileUtils.exists(
    ...
    if (requirementsTxt || pyprojectToml || setupPy) {
      ...
      const pythonVersionFile = await this.fileUtils.readFile(
    ...
    const pomXml = await this.fileUtils.exists(
    ...
    const buildGradle =
      (await this.fileUtils.exists(path.join(projectPath, "build.gradle"))) ||
    ...
    const cargoToml = await this.fileUtils.exists(
  ...
  private async detectFrameworkAndCommands(
    projectPath: string,
    info: ProjectInfo,
  ): Promise<void> {
    const pkg = info.files.packageJson as Record<string, unknown> | undefined;
    ...
    if (info.language === "java") {
      ...
      const pomContent = info.files.pomXml
        ? (await this.fileUtils.readFile(
            path.join(info.rootPath, "pom.xml"),
          )) || ""
        : "";
    ...
    if (info.language === "go") {
      ...
      const goModContent =
        (await this.fileUtils.readFile(path.join(info.rootPath, "go.mod"))) ||
        "";
  ...
  private async detectPackageManager(
    ...
    if (await this.fileUtils.exists(path.join(projectPath, "bun.lockb"))) {
      ...
      await this.fileUtils.exists(path.join(projectPath, "pnpm-lock.yaml"))
    ) {
      info.packageManager = "pnpm";
    } else if (
      await this.fileUtils.exists(path.join(projectPath, "yarn.lock"))
    ) {
      info.packageManager = "yarn";
    } else if (
      await this.fileUtils.exists(path.join(projectPath, "package-lock.json"))
    ) {
      info.packageManager = "npm";
    } else if (
      await this.fileUtils.exists(path.join(projectPath, "poetry.lock"))
    ) {
      info.packageManager = "poetry";
    } else if (
      await this.fileUtils.exists(path.join(projectPath, "Pipfile.lock"))
    ) {
  ...
  private async detectPort(
    ...
    const envExample =
      (await this.fileUtils.readFile(path.join(projectPath, ".env.example"))) ||
    ...
    if (info.entryPoint) {
      const entryContent = await this.fileUtils.readFile(
        path.join(projectPath, info.entryPoint),
      );

      if (entryContent) {
        // Look for patterns like: .listen(3000) or port = 3000 or PORT || 3000
        const portMatch =
          entryContent.match(/\.listen\((\d{4,5})\)/) ||
  ...
  private async detectMonorepo(
    ...
    // Turborepo — has turbo.json
      ...
      await this.fileUtils.exists(path.join(projectPath, "turbo.json"))
    ) {
      info.isMonorepo = true;
    ...
    // Lerna — has lerna.json
      ...
      await this.fileUtils.exists(path.join(projectPath, "lerna.json"))
    ) {
      info.isMonorepo = true;
    ...
    if (pkg?.workspaces) {
      ...
      if (Array.isArray(pkg.workspaces)) {
        info.workspaces = pkg.workspaces as string[];
    ...
    if (info.isMonorepo) {
      info.warnings.push(
        `Monorepo detected (${info.monorepoTool}). ` +
  ...
  private async detectExistingInfrastructure(
    projectPath: string,
    info: ProjectInfo,
  ): Promise<void> {
    info.hasDockerfile = await this.fileUtils.exists(
      path.join(projectPath, "Dockerfile"),
    );
    info.hasDockerCompose =
      (await this.fileUtils.exists(
    ...
    if (info.hasDockerfile) {
      this.logger.info(
        "Existing Dockerfile found — will use it or generate alongside it",
      );
    }
  }
  ...
  private async detectTests(
    ...
    if (info.language === "python") {
      const hasPytest =
        (await this.fileUtils.exists(path.join(projectPath, "pytest.ini"))) ||
