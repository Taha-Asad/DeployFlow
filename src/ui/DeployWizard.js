...
class DeployWizard {
    ...
    async show(extensionUri) {
        return new Promise((resolve) => {
            this.panel = vscode.window.createWebviewPanel("deployflowWizard", "\u{1F680} DeployFlow \u2014 Setup Wizard", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true, localResourceRoots: [extensionUri] });
                    ...
                    if (message.command === "deploy" && message.config) {
                        if (message.credentials) {
                            if (message.config.target === "vps") {
                                ...
                                const ssh = { host: creds.SSH_HOST || "", port: parseInt(creds.SSH_PORT) || 22, username: creds.SSH_USER || "", password: creds.SSH_PASS || undefined };
                                if (ssh.host && ssh.username)
                                    await this.secretManager.storeSshCredentials(ssh);
                            }
                            else {
                                await this.secretManager.storeCloudCredentials({
                                    provider: message.config.target,
...
exports.DeployWizard = DeployWizard;
//# sourceMappingURL=DeployWizard.js.map