...
class AwsDeployer extends BaseDeployer_1.BaseDeployer {
    ...
    async deploy(projectInfo, config, credentials, onProgress) {
        try {
            ...
            if (!awsExists) {
                throw new Error("AWS CLI is not installed. Install it from: https://aws.amazon.com/cli/");
            }
            // ── 2. Get AWS account ID if not provided ─────────────────────────
            let awsAccountId = accountId;
            if (!awsAccountId) {
                onProgress("🔍 Fetching AWS account ID...");
                const idResult = await this.shell.run("aws sts get-caller-identity --query Account --output text", { env });
                if (!idResult.success)
            ...
            const loginResult = await this.shell.run(`aws ecr get-login-password --region ${region} | docker login --username AWS --password-stdin ${awsAccountId}.dkr.ecr.${region}.amazonaws.com`, { env });
            if (!loginResult.success)
                throw new Error("ECR login failed");
            // ── 4. Create ECR repository if it doesn't exist ──────────────────
            onProgress("📦 Ensuring ECR repository exists...");
            await this.shell.run(`aws ecr describe-repositories --repository-names ${appName} --region ${region} || ` +
                `aws ecr create-repository --repository-name ${appName} --region ${region}`, { env });
            ...
            if (!pushResult.success)
                throw new Error("Docker push to ECR failed");
            // ── 6. Update ECS service to use new image ─────────────────────────
            onProgress(`⚙️ Updating ECS service: ${serviceName}...`);
            const updateResult = await this.shell.run(`aws ecs update-service ` +
                `--cluster ${clusterName} ` +
                `--service ${serviceName} ` +
            ...
            await this.shell.run(`aws ecs wait services-stable ` +
                `--cluster ${clusterName} ` +
                `--services ${serviceName} ` +
                `--region ${region}`, { env, timeout: 600000 });
...
exports.AwsDeployer = AwsDeployer;
//# sourceMappingURL=Awsdeployer.js.map