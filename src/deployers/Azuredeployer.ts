...
export class AzureDeployer extends BaseDeployer {
  ...
  public async deploy(
    ...
    try {
      ...
      if (!loginResult.success) throw new Error("Azure login failed");

      await this.shell.run(
        `az account set --subscription ${credentials["AZURE_SUBSCRIPTION_ID"]}`,
      );
      ...
      if (!pushResult.success) throw new Error("Docker push to ACR failed");

      // ── 5. Create or update Container App ─────────────────────────────
      onProgress("🚀 Deploying to Azure Container Apps...");

      // Check if container app already exists
      const existsResult = await this.shell.run(
        `az containerapp show --name ${appName} --resource-group ${resourceGroup} --query name -o tsv`,
      );
      ...
      if (existsResult.success) {
        ...
        await this.shell.run(
          `az containerapp env show --name ${envName} --resource-group ${resourceGroup} || ` +
      ...
      // ── 6. Get deployed URL ────────────────────────────────────────────
      const fqdnResult = await this.shell.run(
