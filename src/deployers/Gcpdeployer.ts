...
export class GcpDeployer extends BaseDeployer {
  ...
  public async deploy(
    ...
    try {
      ...
      if (!gcloudExists) {
        throw new Error(
          "gcloud CLI is not installed. Install from: https://cloud.google.com/sdk",
        );
      }
      ...
      if (credentials["GCP_SA_KEY_FILE"]) {
        ...
        await this.shell.run(
          `gcloud auth activate-service-account --key-file=${credentials["GCP_SA_KEY_FILE"]}`,
        );
      }
      ...
      await this.shell.run(
        `gcloud artifacts repositories describe ${appName} ` +
      ...
      if (!pushResult.success) throw new Error("Docker push failed");

      // ── 6. Deploy to Cloud Run ─────────────────────────────────────────
      onProgress("🚀 Deploying to Cloud Run...");
      const deployResult = await this.shell.run(
      ...
      const urlResult = await this.shell.run(
        `gcloud run services describe ${appName} ` +
          `--region=${region} --project=${projectId} ` +
          `--format="value(status.url)"`,
      );

      const deployedUrl =
        urlResult.stdout.trim() || `https://${appName}-${projectId}.run.app`;
