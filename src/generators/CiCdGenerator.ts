...
`;
    ...
    await this.fileUtils.writeFile(
      path.join(info.rootPath, ".gitlab-ci.yml"),
      gitlabCi,
    );
  }

  // ── Generate setup steps for GitHub Actions ───────────────────────────
  private generateSetupSteps(info: ProjectInfo): string {
  ...
  private getInstallCmd(info: ProjectInfo): string {
  ...
  private getGitLabImage(info: ProjectInfo): string {
    if (info.language === "python")
  ...
  private getGitLabInstallCmd(info: ProjectInfo): string {
    if (info.language === "python") return `- pip install -r requirements.txt`;
    if (info.language === "go") return `- go mod download`;
