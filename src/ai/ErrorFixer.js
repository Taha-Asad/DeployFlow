...
class ErrorFixer {
    ...
    logger;
    configManager;
    constructor(aiManager, configManager) {
    ...
    async fixBuildErrors(projectPath, errorOutput, buildFn, onProgress, showDiff) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            onProgress(`🤖 AI Fix Attempt ${attempt}/${maxAttempts}: Analyzing error...`);
            ...
            const contextFiles = await this.gatherContextFiles(projectPath, currentError);
            // ── 2. Ask AI for a fix ──────────────────────────────────────
            let fixResponse;
            try {
                fixResponse = await this.aiManager.fixBuildError(currentError, contextFiles);
            }
            catch (aiError) {
            ...
            if (fixResponse.patches.length === 0) {
                onProgress("⚠️ AI could not determine a fix. Check the error manually.");
                return {
                    success: false,
                    totalAttempts: attempt,
                    finalError: currentError,
            ...
            onProgress(`⚠️ Build still failing after fix. ${attempt < maxAttempts ? "Trying again..." : "Max attempts reached."}`);
        }
        return {
            success: false,
    ...
    async fixDeployErrors(errorOutput, runRemoteCommand, deployFn, onProgress) {
        ...
        let currentError = errorOutput;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            onProgress(`🤖 AI Deploy Fix Attempt ${attempt}/${maxAttempts}: Analyzing error...`);
            ...
            for (const cmd of fixResponse.remoteCommands) {
                ...
                const approve = await vscode.window.showInformationMessage(`${prefix} AI suggests: ${cmd.description}\n\nCommand: ${cmd.command}`, { modal: true }, "Run", "Skip");
                if (approve === "Run") {
            ...
            if (deployResult.success) {
                onProgress(`✅ Deployment succeeded after ${attempt} fix attempt(s)!`);
                return {
                    success: true,
                    totalAttempts: attempt,
                    fixedBy: `attempt-${attempt}`,
            ...
            onProgress(`⚠️ Deployment still failing after fix. ${attempt < maxAttempts ? "Trying again..." : "Max attempts reached."}`);
        }
        return {
            success: false,
            totalAttempts: maxAttempts,
    ...
    async saveDeployErrorLog(attempt, errorOutput, fix) {
        try {
            const logsDir = path.join(this.configManager.getWorkspaceFolder() || ".", ".deployflow", "error-logs");
            const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
            const logPath = path.join(logsDir, `deploy-error-attempt-${attempt}-${timestamp}.json`);
            const logData = {
    ...
    MAX_CONTEXT_SIZE = 50_000; // characters total
    MAX_FILE_SIZE = 8_000; // characters per file
    async gatherContextFiles(projectPath, errorOutput) {
        const files = new Map();
        let totalSize = 0;
        // Helper: add a file if we haven't hit the budget cap
        const addFile = (name, content) => {
            if (files.has(name) || totalSize >= this.MAX_CONTEXT_SIZE)
                return;
            const truncated = content.length > this.MAX_FILE_SIZE
        ...
        const filePattern = /(?:^|\s)([./\w-]+\.(?:ts|tsx|js|jsx|py|java|go|rs|php))/gm;
        ...
        this.logger.debug(`Gathered ${files.size} context files (${totalSize} chars) for AI analysis`);
        return files;
    }
    ...
    async applyPatches(projectPath, patches, onProgress) {
        for (const patch of patches) {
            ...
            if (existing) {
                await this.fileUtils.writeFile(`${fullPath}.deployflow-backup`, existing);
            }
    ...
    async saveErrorLog(projectPath, attempt, errorOutput, fix) {
        try {
            const logsDir = path.join(projectPath, ".deployflow", "error-logs");
            const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
            const logPath = path.join(logsDir, `error-attempt-${attempt}-${timestamp}.json`);
            const logData = {
                timestamp: new Date().toISOString(),
                attempt,
                errorOutput,
...
//# sourceMappingURL=ErrorFixer.js.map